<div>
    ...
    <x-form-section submit="createApiToken">
        ...
        <x-slot name="description">
            {{ __('API токени дозволяють сторонім сервісам автентифікуватися з нашим додатком від вашого імені.') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-label for="name" value="{{ __('Назва токена') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-label for="permissions" value="{{ __('Дозволи') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                ...
                <x-slot name="description">
                    {{ __('Ви можете видалити будь-які з ваших існуючих токенів, якщо вони більше не потрібні.') }}
                </x-slot>
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ __('Будь ласка, скопіюйте ваш новий API токен. Для вашої безпеки він більше не буде показаний.') }}
            </div>
    ...
    <x-confirmation-modal wire:model.live="confirmingApiTokenDeletion">
        <x-slot name="title">
            {{ __('Видалити API токен') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Ви впевнені, що хочете видалити цей API токен?') }}
        </x-slot>
