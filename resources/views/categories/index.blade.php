<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @if(auth()->check() && auth()->user()->can('create categories'))
                <a href="{{ route('teacher.categories.create') }}" class="inline-flex items-center px-4 py-2 btn-gradient border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition ease-in-out duration-150">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6">
                <!-- Фільтри -->
                <div class="mb-6">
                    <form action="{{ route('categories.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                            <x-label for="search" value="{{ __('Пошук за назвою') }}" />
                        ...
                        <div>
                            <x-label for="subject" value="{{ __('Предмет') }}" />
                            <select id="subject" name="subject" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full">
                ...
                @if($categories->isEmpty())
                    <div class="text-center py-12">
                        ...
                        <h3 class="mt-2 text-lg font-medium text-gray-900 dark:text-white">Категорій не знайдено</h3>
