...
class EventStartingSoonNotification extends Notification implements ShouldQueue
{
    ...
    /**
     * Подія, яка починається через 2 дні
     *
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->subject('Нагадування: Подія починається через 2 дні - ' . $this->event->name)
            ->line('Шановний(а) ' . $notifiable->full_name . ',')
            ->line('Нагадуємо вам, що через 2 дні починається подія: **' . $this->event->name . '**')
            ...
            ->line('Будь ласка, підготуйтеся до участі в події.')
