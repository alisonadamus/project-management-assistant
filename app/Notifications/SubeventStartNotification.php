...
class SubeventStartNotification extends Notification implements ShouldQueue
{
    use Queueable;

    /**
     * Підподія, яка починається
     *
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->subject('Початок підподії: ' . $this->subevent->name)
            ->line('Шановний(а) ' . $notifiable->full_name . ',')
            ->line('Повідомляємо вас, що сьогодні починається підподія: **' . $this->subevent->name . '**')
